...
# requests.post("192.168.137.1:8000/api/updatepersoncount", data = {'count':10}, headers=headers)

# Function to send the count to Django server
def send_count_to_django():
    count = django_person_count
    url = 'http://192.168.252.44:8000/api/updatepersoncount'  # Replace with your Django view URL
    data = {'count': count}
    try:
...
#function to send data to backend django server using the below script
...
def people_counter():
    ...
    while True:
        ...
        info_total = [
            ("Total people inside", (totalDown - totalUp)*-1),
        ]
        ...
        global django_person_count
        django_person_count = totalDown - totalUp
    ...
    # Send the final count to Django server




if __name__ == "__main__":
    schedule.every(5).seconds.do(send_count_to_django)
    people_counter()
